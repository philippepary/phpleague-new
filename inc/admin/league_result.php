...
$pagination = $fct->pagination($nb_fixtures, 1, $fixture_number, 'id_fixture');
...
$output .= '</div><table class="widefat"><thead><tr><th colspan="6">'.$league_name.
        __(' - Fixture: ', 'phpleague') . $fixture_number . '</th></tr></thead>';
foreach ($db->get_results_by_fixture($fixture_number, $id_league) as $key => $row)
{
    $output .= '<tr>'
            .'<td style="text-align:right;" class="team-column">'.esc_html($row->name_home).'</td>';
    ...
    $output .= '<td class="check-column" class="team-column">'.$fct->input('array['.$key.'][date]',
            esc_html($row->played), array('size' => 18, 'class' => 'masked-full', 'type' => 'hidden')).$fct->input('array['.$key.'][id_match]',
