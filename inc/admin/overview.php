...
elseif (isset($_POST['add_league']) && check_admin_referer('phpleague')) // Add a new league
{
    ...
    $sport = ( ! empty($_POST['sport'])) ? $_POST['sport'] : NULL;
    ...
    elseif ( ! in_array($sport, array_keys(PHPLeague_Sports::$sports)) )
    {
    	$message[] = __('The sport is not known', 'phpleague');
    }
    else
    {
        $db->add_league($name, $year, $sport);
...
// tim modified - 1
$output .= $fct->input('name', "", array('class' => 'default', 'placeholder' => __('Name', 'phpleague')));
$output .= $fct->input('year', "", array('class' => 'default', 'placeholder' => __('Year', 'phpleague')));
$sports = array();
foreach (PHPLeague_Sports::$sports as $key => $value) {
    $sports[$key] = __($key, 'phpleague');
}
$output .= $fct->select('sport', $sports, null, array('class' => 'default'));
...
$output .= '</div><table class="widefat"><thead><tr>'
    ...
    foreach ($db->get_every_league($offset, $per_page) as $league)
    {
        ...
        $matches = "<td></td>";
        $results = "<td></td>";
        if ( count( $db->get_every_club_in_league( $league->id ) ) > 1 )
        {
            $fixtures = '<td><a href="'.admin_url($page_url.'&option=fixture&id_league='.$id).'">'.__('Fixtures', 'phpleague');
            if ( count ( $db->get_fixtures_league($league->id) ) > 0 )
            {
                $matches = '</a></td><td><a href="'.admin_url($page_url.'&option=match&id_league='.$id).'">'.__('Matches', 'phpleague'); 
                $results = '</a></td><td><a href="'.admin_url($page_url.'&option=result&id_league='.$id).'">'.__('Results', 'phpleague');
            }
        }
        
        $output .= '<tr '.$fct->alternate('', 'class="alternate"').'>'
                .'<th class="check-column"><input type="checkbox" name="id_league[]" value="'.$id.'" /></th>'
                .'<td>'.esc_html($league->id).'</td>'
