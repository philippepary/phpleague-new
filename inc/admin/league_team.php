...
// Menu
$menu = array(__('Teams', 'phpleague')    => '#');
if ( count( $db->get_every_club_in_league( $id_league ) ) > 1 )
{
    $menu[__('Fixtures', 'phpleague')] = admin_url('admin.php?page=phpleague_overview&option=fixture&id_league='.$id_league);
...
$output .= $fct->select('id_club', $clubs_list).$fct->input('add_club', __('Add', 'phpleague'),
        array('type' => 'submit', 'class' => 'button'));
$output .= sprintf(__(' Your league has currently <b>%d</b> teams.', 'phpleague'), $total);
