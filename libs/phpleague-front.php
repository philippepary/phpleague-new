...
if ( ! class_exists('PHPLeague_Front')) {
    ...
    class PHPLeague_Front {
        ...
        public function get_league_fixtures($id_league, $id_team = NULL)
        {
            ...
            {
                $output = '<table id="phpleague" class="team-fixtures-table"><tbody><tr>'
                    .'<th class="centered">'.__('Fixture', 'phpleague').'</th>'
                    .'<th class="centered">'.__('Date', 'phpleague').'</th>'
                    .'<th class="centered">'.__('Match', 'phpleague').'</th>'
                    .'<th class="centered">'.__('Score', 'phpleague').'</th></tr>';
            ...
            else // No team has been given...
            {
                ...
                foreach ($db->get_fixtures_by_league((int) $id_league) as $row)
                {
                    if ($current_num != $row->number)
                    {
                        ...
                        $start_date->sub(new DateInterval("P".($day_of_week-1)."D"));
                        $end_date->add(new DateInterval("P".(7-$day_of_week)."D"));
                        $output .= '<table id="phpleague" class="fixtures-table"><tbody><tr>'
                                .'<th colspan="3">'.__('Fixture', 'phpleague').' '.(int) $row->number
                                .' ('.date_i18n(get_option('date_format'), $start_date->getTimestamp())." - ".date_i18n(get_option('date_format'), $end_date->getTimestamp()).')</th>';
                        $output .= '</tr>';
                    }
                    $output .= '<tr>';
