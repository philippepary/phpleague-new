...
if ( ! class_exists('PHPLeague_Database')) {
    ...
    class PHPLeague_Database extends PHPLeague_Tools {
         ...
         * @return object
         */
        public function update_club_information($id_club, $name, $id_country, $coach, $venue, $creation, $website, $logo_b, $logo_m, $wp_user_id, $address, $phone, $training)
        {           
            global $wpdb;
            return
            $wpdb->update( 
        ...
        {
            global $wpdb;
            return $wpdb->get_row($wpdb->prepare("SELECT name, venue, coach, id_country, logo_big, logo_mini, creation, website, wp_user_id, address, phone, training_time
                FROM $wpdb->club
                WHERE id = %d",
                $id)
            );
        }
        public function get_user_clubs_information($wp_user_id)
        {
            global $wpdb;
            return $wpdb->get_results($wpdb->prepare("SELECT id, name, venue, coach, id_country, logo_big, logo_mini, creation, website, wp_user_id, address, phone, training_time
                FROM $wpdb->club
                WHERE wp_user_id = %d",
                $wp_user_id)
            );
         ...
         * @return object
         */
        public function add_league($name, $year, $sport)
        {           
            global $wpdb;
            return $wpdb->insert($wpdb->league, array('name' => $name, 'year' => $year, 'sport' => $sport), array('%s', '%d', '%s'));
        }
        ...
        {
            global $wpdb;
            return $wpdb->get_results($wpdb->prepare("SELECT number, scheduled, id, non_playing_team_id
                FROM $wpdb->fixture
                WHERE id_league = %d
         ...
         * @return object
         */
        public function edit_league_fixtures($number, $scheduled, $id_league, $not_playing)
        {
            global $wpdb;
            return
            $wpdb->update( 
                $wpdb->fixture, 
                array('scheduled' => $scheduled, 'non_playing_team_id' => $not_playing), array('id_league' => $id_league, 'number' => $number), 
                array('%s', '%d'), array('%d', '%d') 
            );
        }
        ...
        /** Get team id for a given club in a given league.
         * @return Team id or null if not assigned to the league
         */
